...
The recommended way to build the Android APK is using the Tauri 2 app in `src-tauri2`, which uses the same codebase as the Windows desktop build.

* Prerequisites: Rust (with `wasm32-unknown-unknown` target), link:https://trunkrs.dev[Trunk], Tauri CLI v2 (`cargo install tauri-cli --version 2`), and the Android SDK/NDK.
* From the repository root, initialize Android support once, then build the APK:
+
----
cd src-tauri2
...
----
+
The APK is produced under `src-tauri2/gen/android/app/build/outputs/apk/`. The legacy Cordova build in `cube_shuffle-cordova` is deprecated in favor of Tauri 2.
